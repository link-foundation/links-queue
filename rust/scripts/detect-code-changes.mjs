...
/**
 * Detect code changes for CI/CD pipeline (Rust)
 *
...
function isExcludedFromCodeChanges(filePath) {
  // Only consider files in the rust/ folder
  if (!filePath.startsWith('rust/')) {
    return true;
  }

  // Get relative path within rust folder
  const relativePath = filePath.replace(/^rust\//, '');
...
function detectChanges() {
  console.log('Detecting file changes for CI/CD (Rust)...\n');

  const changedFiles = getChangedFiles();

  // Filter to only rust/ folder files
  const rustChangedFiles = changedFiles.filter((file) =>
    file.startsWith('rust/')
  );

  console.log('Changed files in rust/:');
  if (rustChangedFiles.length === 0) {
    console.log('  (none)');
  } else {
    rustChangedFiles.forEach((file) => console.log(`  ${file}`));
  }
  console.log('');

  // Detect .rs file changes (Rust source)
  const rsChanged = rustChangedFiles.some((file) => file.endsWith('.rs'));
  setOutput('rs-changed', rsChanged ? 'true' : 'false');

  // Detect .toml file changes (Cargo.toml, Cargo.lock, etc.)
  const tomlChanged = rustChangedFiles.some((file) => file.endsWith('.toml'));
  setOutput('toml-changed', tomlChanged ? 'true' : 'false');

  // Detect .mjs file changes (scripts)
  const mjsChanged = rustChangedFiles.some((file) => file.endsWith('.mjs'));
  setOutput('mjs-changed', mjsChanged ? 'true' : 'false');

  // Detect documentation changes (any .md file)
  const docsChanged = rustChangedFiles.some((file) => file.endsWith('.md'));
  setOutput('docs-changed', docsChanged ? 'true' : 'false');

  // Detect workflow changes
  const workflowChanged = changedFiles.some(
    (file) => file.startsWith('.github/workflows/') && file.includes('rust')
  );
  ...
  const codeChangedFiles = rustChangedFiles.filter(
  ...
  const codePattern = /\.(rs|toml|mjs|yml|yaml)$|\.github\/workflows\//;
